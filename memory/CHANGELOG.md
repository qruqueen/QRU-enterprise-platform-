...
## 2026-07-29 — Bundle Cover Studio™ + Publish Preview Everywhere + Imprint Rollout (testing_agent iteration_99 — 11/11 backend pass, all frontend verified, 0 blocking issues)
- **Bundle Cover Studio™:** `bundles.render_bundle_cover_bytes()` composes a deterministic ($0 AI) LANDSCAPE branded cover (QRU • CURATED BUNDLE, shield, stack-of-books motif, title auto-fit, N-TITLES + SAVE-$X pills, QRU PRESS™ band). `generate_cover()` saves via `re_engine._save` → cover_url `/api/rendering/asset/bundle-cover-*.png`. Auto-generates on create + publish; manual `POST /api/bundles/{id}/cover`. BundlesManager shows the cover + a "Regenerate cover" button. Bundles no longer borrow the first book's cover.
- **Publish Preview Everywhere:** `DestinationChips` wired into ProductFamily (`family-destinations` — each selected family → its auto destinations from the map) and MediaStudio (`media-catalog-destination` — Media experience). (ProductShelf + Book authorize step already had it from Phase 1.)
- **Imprint Rollout:** `set_publication_details` + `PublicationDetailsReq` now persist `imprint`; Book Manufacturing identity editor has an imprint `<select>` (QRU Press™ / E.Q. Rothwell™). `GET /api/public/books?imprint=` filters + returns `imprints`. Assigned E.Q. Rothwell™ to 3 preview books → public Books filter now shows both imprints live (8 total / 3 E.Q. Rothwell™). `design_language.premium_cover` made imprint-aware for future renders.
- KNOWN COSMETIC CAVEAT (documented, non-blocking): existing book cover ARTWORK bakes the original imprint badge; changing imprint metadata does not re-render the baked PNG (covers are baked assets). Byline/metadata + filter are correct. A future 'Re-render covers for imprint' action could reconcile the baked badge. Pre-existing dev-only `<span> in <option>` hydration warning remains (noise only).


## 2026-07-29 — Phase 1: Books Experience + First-Class Bundles + Publish Destination Preview (testing_agent iteration_98 — 17/17 backend pass, frontend verified, 0 blocking issues)
Founder-approved defaults (1a/2a/3a/4a/5-both). Live public site + live Stripe path — checkout tested at SESSION-CREATION only (no real payment).
- **Books Experience (shell + imprint filter):** public `PublicLayout` now has Home / Books / Bundles tabs (experience shell). `QRUCatalog` renders imprint filter chips when 2+ imprints exist. `GET /api/public/books` accepts `?imprint=` and returns `{books,count,imprints}`. (All current books are QRU Press™; E.Q. Rothwell™ slots in via book_records.imprint — filter auto-appears when a 2nd imprint exists.)
- **First-Class Bundles:** new `bundles.py` engine + `routers/bundles.py` (Founder CRUD, super-admin) + `routers/public_bundles.py` (public list/detail/checkout/status/access/download). A Bundle is a real product: explicit Founder price, savings vs. sum shown, books-only for now (data model accepts any type; `deliverable` flag + publish guard require all items deliverable). Single Stripe purchase → server-side pricing → idempotent `_fulfill` (72h window, access_id, order_ref) → one confirmation email → `/bundle-access/{token}` lists each item's secure `/bundle-download/{access_id}/{book_id}` (paid + window + per-item cap 5). Founder page `BundlesManager.js` (`/bundles`, nav "Bundles™", capability registered) create/publish/unpublish/delete with live items-total. Public pages `QRUBundles`, `QRUBundlePage`, `QRUBundleAccess`; `QRUPurchaseSuccess` handles `kind=bundle`.
- **Publish Destination Preview:** `components/DestinationPreview.js` → `DestinationPreview` (editable; loads `GET /api/distribution-architecture/destinations?target=&id=`, override via `POST` with empty list = revert to auto) wired into Book Manufacturing authorize step (target=book); `DestinationChips` (read-only, from destinations-map) wired per-row into `ProductShelf`. New endpoints GET/POST `/api/distribution-architecture/destinations` (`_TARGET_COLL`: product→products, book→book_records).
- Non-blocking: pre-existing dev-only React hydration warning in the BookManufacturing book-switcher `<select>` (x-source debug attrs), unrelated to this work — left as-is.
- STILL DEFERRED (later phases): public visibility of non-book products (Resources/Media experiences on qru-online.com), video/motion/poster rendering, mixed-type bundle purchasability, Audiobook Storefront.
...
- **Deliverable 1 — Operator's Manual**: full 18-field detail (purpose, why, inputs/outputs, records created/modified/consumed, predecessor/successor, Founder-required, auto/manual, use cases, example, common misunderstanding, status) for ~17 core launch capabilities + a by-department catalog of every other implemented capability. Business language.
- **Deliverable 2 — Quick Start**: 11 "I have X, what next?" workflows (manuscript, import, KR, book, product family, cover, publish, update, remove/relist, deliver, clean/upgrade live store) with Start → decision → Next → output → Done.
- **Deliverable 3 — Relationship Map**: 6 lanes (Knowledge → Understanding/Manufacturing → Assessment → Publishing/Catalog → Customer-facing Delivery → always-on Governance/Memory/Health), each node showing In/Out/Next. Plus the explicit **QRU Store™ (internal catalog capability) vs qru-online.com (customer website)** distinction — CONFIRMED as intended, with the honest nuance that authorizing/publishing lists in QRU Store™ AND surfaces on qru-online.com together (the site's live-gate reads the book's Founder-authorization flag).
- Observations (documented, NOT a redesign): merged/duplicate capabilities to avoid, Future-Backlog items, preview-vs-prod DB separation, Audiobook Storefront not yet built. Verified: all 3 tabs render, no console errors. Memory: `/app/memory/founder_docs/README.md`.
