## ✅ 2026-08-05 (cont.): Durable mirror verification + Cover Health dashboard (production-prevention)
- **Durable mirror verification (`storage.mirror_file`)**: a successful PUT is no longer treated as success — the mirror now VERIFIES retrieval with a ranged GET and retries once, returning True only when the object is confirmed retrievable. This closes the exact gap that lost the concept-1 cover masters (PUT "ok" but object not retrievable). Never raises (won't fail a render). Verified: real put → True; missing object → False.
- **Per-concept durable stamp (`design_studio.manufacture_design_concepts`)**: every generated cover concept now carries `durable_verified` (live `aobject_exists` check) so a cover is only surfaced as durable-confirmed. Combined with the earlier `select_cover`/`upload_cover` safeguard, a cover cannot become the active selection unless its durable upload + retrieval are verified.
- **Cover Health dashboard (Store Health™)**: `store_health._cover_integrity()` does a LIVE durable check of every published book's active cover (reuses `pm._asset_ok`), added to `/api/store/health` as `cover_integrity` + a `covers_durable` integrity check. Frontend `StoreHealth.js` new "Cover Integrity" section: checked / broken / repairable / unrepairable stats + per-book rows (code, title, active cover filename, Fallback→concept N badge) + guidance to run Cover Reference Repair. Verified: preview shows 9 checked / 2 broken (BOOK-0001, BOOK-0011, both repairable→concept 2); BOOK-0012 correctly excluded (not published). Screenshot confirmed.
- Deferred per Founder: Auto AI re-generation of lost concepts (later milestone). ⚠️ PREVIEW — redeploy to activate in production; Cover Health then reflects the production DB live.


## ✅ 2026-08-05: PRODUCTION cover 404/500 — root-caused & fixed (resilient resolution + Founder Repair + durable safeguard)
- **Root cause**: For 3 books (BOOK-0001 The Understanding Tree, BOOK-0011 The Heart as a Daily Circulation Pump, BOOK-0012 Day Trading as a Decision System) the Founder-SELECTED cover (concept 1) master PNG is absent from durable object storage (durable GET returns HTTP **500**, not 404), while concepts 2/3 for the same books are intact (200). `rendering_engine._save` mirrors best-effort, so concept 1's durable mirror silently failed at generation and the ephemeral copy was later pruned → `cover-thumb` → `ensure_local`→`get_object` raised → route returned 404 and re-hit the broken durable key on every homepage load (retry storm). Durable storage is a shared `qru-online` namespace, so a pure code fix heals production on redeploy.
- **Fix 1 — Resilient cover resolution (`routers/public_site.py`)**: `cover-thumb` now tries the selected concept FIRST, then falls back to the next Founder-generated concept whose master is actually retrievable. **Negative cache** (`_missing_masters`, 600s TTL) stops re-hitting a known-broken durable key every request (kills the 500 storm) and self-heals (selected concept retried first, so it auto-restores if it returns). When NO concept is retrievable → a controlled, cached branded **placeholder** JPEG (HTTP 200, short cache) instead of a broken 404. Verified: both reported books + BOOK-0012 now return 200 JPEG covers; all catalog covers 200.
- **Fix 2 — Founder Repair (Production Operations™)**: `prod_migrations.cover_repair_preflight/cover_repair/cover_repair_rollback` + `/api/admin/migrations/cover-repair[/preflight|/rollback]` + a "Cover Reference Repair" card (Audit / Dry Run / Repair / Rollback, testids `cov-*`, `cover-repair`). Permanently repoints a book's active `selected_cover` to the first retrievable concept; preserves original in `artifacts.design.cover_repair.preimage`; idempotent + reversible. Verified apply (3 REPAIRED), idempotent re-apply (no-op), rollback (RESTORED), UI screenshot.
- **Fix 3 — Preventative durable safeguard (`book_manufacturing._verify_cover_durable`)**: `select_cover` and `upload_cover` now REFUSE to write a cover as the active selected cover until its master is confirmed in durable storage (upload succeeded + retrieval verified; re-mirrors from local + re-verifies). Verified: selecting the broken concept 1 is rejected with an honest error; a durable concept succeeds.
- Preview left in original state (concept 1 selected) to mirror production; display fallback keeps the storefront healthy. ⚠️ PREVIEW fix — Founder must **redeploy**, then optionally run the Cover Reference Repair operation on qru-online.com for a permanent DB fix.
